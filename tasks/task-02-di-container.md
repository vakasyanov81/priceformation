...
- [x] Все зависимости конструируются через `ServiceProvider`.
- [x] `configure_services()` вызывается один раз при старте приложения.
- [x] Тесты не зависят от глобального контейнера (reset после каждого теста).
- [x] `make_parser()` удалён или переведён на `ServiceProvider`.