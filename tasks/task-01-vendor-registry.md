...
- [ ] Добавление нового вендора = создание класса + `@register_vendor`, **без правки** `all_vendors.py` и `common_price.py`.
- [ ] `all_vendors()` возвращает тот же набор вендоров, что и до рефакторинга.
- [ ] Тесты `test_all_vendors` проходят без изменений.
- [ ] Старые прямые импорты вендоров удалены (кроме тех, что нужны в тестах).