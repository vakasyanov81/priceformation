...
Отпускная цена **равна закупу**: это внутренние остатки склада, цена в прайсе уже с наценкой. Правил наценки нет и JSON не нужен. Присвоение `price_markup = price_opt` спрятано в оркестрации рядом с разбором title. `get_min_rest_count` = 0 (не отсекать малый rest) — не путать с наценкой.
