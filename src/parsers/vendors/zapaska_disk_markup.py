...
def _is_small_recommended_price(price_recommended: float, price_opt: float, percent: float) -> bool:
    ...
    return bool(price_recommended and BaseParser.calc_percent(price_recommended, price_opt) <= percent)
...
def _make_price_recommended_markup(
    ...
    percent = BaseParser.calc_percent(price_recommended, price_opt)
    ...
    return BaseParser.get_markup(price_opt, percent), percent
...
def make_price_markup_value(price_recommended: float, price_opt: float) -> float:
    ...
    if not price:
        price = BaseParser.get_markup(price_opt, _get_price_percent_markup(price_opt))
